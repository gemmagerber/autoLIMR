...
LIM_Read <-
            ...
            remtabs = TRUE)
  {
    ...
    SplitEquation <- function(string) {
      ...
      ii <- which(type == "FALSE")
      if (length(ii) > 0)
        stop(paste("cannot proceed: following is not an equation: ",
                   string[ii]))
    ...
    ParseLine <- function(String,
      ...
      for (i in 1:length(String)) {
        ...
        for (j in 1:length(ss))
          cost <- rbind(cost, data.frame(name = names[i],
    ...
    allnames <- c(parnames, varnames, externnames, compnames,
                  reacnames[, 1])
    ...
    if (length(equisec) > 1) {
      ...
      if (length(grep(":", eqstrings, fixed = TRUE)) > 0) {
        ...
        equis <- ParseEquation(SplitEquation(eqstrings),
                               "eq")
    }
    if (verbose)
