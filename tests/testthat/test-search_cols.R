test_that("search_col function returns a vector of given length for each
          Export, Import, AE",
          {
            ...
            # Test Exports
            testex <- search_cols(x = tidy, col_match = "Export")
            expect_vector(object = testex, size = 2)
            for (i in length(testex)) {
              expect_true(grepl(x = testex[i], pattern = paste0(
                c("Export", "Exports", "Ex", "EX", "EX_"), collapse = "|"
              )))
            }

            # Test Imports
            testin <- search_cols(x = tidy, col_match = "Import")
            expect_vector(object = testin, size = 2)
            for (i in length(testin)) {
              expect_true(grepl(x = testin[i], pattern = paste0(
                c(
                  "Import",
                  "Input",
                  "^+In+$",
            ...
            # Test AE
            testae <- search_cols(x = tidy, col_match = "AE")
            expect_vector(object = testae, size = 2)
            for (i in length(testae)) {
              expect_true(grepl(x = testae[i], pattern = paste0(
                c("AE", "Assimilation", "efficiency", "AssEm"), collapse = "|"
              )))
            }
          })
