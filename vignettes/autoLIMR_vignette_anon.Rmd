...
output:
  rmarkdown::html_vignette:
    ...
    fig_caption: true
vignette: >
  %\VignetteIndexEntry{autoLIMR_vignette_anon}
...
## Installation
The latest version of '__autoLIMR__' can be installed from [GitHub](https://b10200cbde7d/r/autoLIMR-084C). __autoLIMR__ dependencies are automatically installed, including cellranger (>= 1.1.0) (Bryan, 2016), readxl (>= 1.4.1) (Wickham and Bryan, 2022), and utils (>= 4.2.1) (R Core Team, 2022). 

```{r, eval = FALSE}
devtools::install_github("XXXX-XXXX/autoLIMR")
library(autoLIMR)
```
...
familiar with R. The latest version of __autoLIM-Excel__ can be download from the [autoLIM-Excel GitHub repository](XXXX-1) or downloaded to the working directory with the autoLIMR function `fetch_autoLIMExcel()` during an active R session. Please set `force = TRUE` to
